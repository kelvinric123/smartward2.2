        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('ot-scheduling.store-booking') }}" class="space-y-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <x-text-input id="schedule_date" type="date" name="schedule_date" :value="old('schedule_date')" required min="{{ now()->format('Y-m-d') }}" class="block mt-1 w-full" />
                                <x-input-error :messages="$errors->get('schedule_date')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="start_time" :value="__('Start Time')" />
                                <x-text-input id="start_time" type="time" name="start_time" :value="old('start_time')" required step="900" class="block mt-1 w-full" />
                                <x-input-error :messages="$errors->get('start_time')" class="mt-2" />
                                <p class="mt-1 text-xs text-gray-600">Time slots are in 15-minute intervals</p>
                            </div>
                            <div>
                                <x-input-label for="end_time" :value="__('End Time')" />
                                <x-text-input id="end_time" type="time" name="end_time" :value="old('end_time')" required step="900" class="block mt-1 w-full" />
                        ...
                        <div>
                            ...
                            <select id="anesthetist_id" name="anesthetist_id" required
                                ...
                                @foreach($anesthetists as $anesthetist)
                                    <option value="{{ $anesthetist->id }}" {{ old('anesthetist_id') == $anesthetist->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-600">Preferably select an available room unless there's a specific requirement.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="procedure_type" name="procedure_type" required
                                ...
                                <option value="Appendectomy" {{ old('procedure_type') == 'Appendectomy' ? 'selected' : '' }}>Appendectomy</option>
                                <option value="Coronary Bypass" {{ old('procedure_type') == 'Coronary Bypass' ? 'selected' : '' }}>Coronary Bypass</option>
                                <option value="Knee Replacement" {{ old('procedure_type') == 'Knee Replacement' ? 'selected' : '' }}>Knee Replacement</option>
                                <option value="Hip Replacement" {{ old('procedure_type') == 'Hip Replacement' ? 'selected' : '' }}>Hip Replacement</option>
                                <option value="Cataract Surgery" {{ old('procedure_type') == 'Cataract Surgery' ? 'selected' : '' }}>Cataract Surgery</option>
                                <option value="Hernia Repair" {{ old('procedure_type') == 'Hernia Repair' ? 'selected' : '' }}>Hernia Repair</option>
                                <option value="Tonsillectomy" {{ old('procedure_type') == 'Tonsillectomy' ? 'selected' : '' }}>Tonsillectomy</option>
                                <option value="Cholecystectomy" {{ old('procedure_type') == 'Cholecystectomy' ? 'selected' : '' }}>Cholecystectomy (Gallbladder Removal)</option>
                                <option value="Hysterectomy" {{ old('procedure_type') == 'Hysterectomy' ? 'selected' : '' }}>Hysterectomy</option>
                                <option value="Brain Tumor Removal" {{ old('procedure_type') == 'Brain Tumor Removal' ? 'selected' : '' }}>Brain Tumor Removal</option>
                                <option value="Thyroidectomy" {{ old('procedure_type') == 'Thyroidectomy' ? 'selected' : '' }}>Thyroidectomy</option>
                                <option value="Mastectomy" {{ old('procedure_type') == 'Mastectomy' ? 'selected' : '' }}>Mastectomy</option>
                                <option value="Other" {{ old('procedure_type') && !in_array(old('procedure_type'), ['Appendectomy', 'Coronary Bypass', 'Knee Replacement', 'Hip Replacement', 'Cataract Surgery', 'Hernia Repair', 'Tonsillectomy', 'Cholecystectomy', 'Hysterectomy', 'Brain Tumor Removal', 'Thyroidectomy', 'Mastectomy']) ? 'selected' : '' }}>Other</option>
                            </select>
                            ...
                            <div id="custom_procedure_container" class="mt-2" style="display: none;">
                                ...
                                <x-text-input id="custom_procedure" type="text" class="block mt-1 w-full" />
