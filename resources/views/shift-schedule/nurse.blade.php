@extends('layouts.app')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold text-gray-800">{{ $nurse->name }}'s Shift Schedule</h2>
            
            <div class="flex space-x-3">
                <a href="{{ route('shift-schedule.index') }}" class="text-indigo-600 hover:text-indigo-900">
                    Back to All Schedules
                </a>
                
                <a href="{{ route('shift-schedule.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">
                    Add New Shift
                </a>
            </div>
        </div>
        
        <!-- Nurse Info -->
        <div class="mb-6 bg-blue-50 p-4 rounded-md">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <span class="block text-sm font-medium text-gray-700">Position</span>
                    <span class="block font-semibold">{{ $nurse->position }}</span>
                </div>
                <div>
                    <span class="block text-sm font-medium text-gray-700">Ward Assignment</span>
                    <span class="block font-semibold">
                        <a href="{{ route('shift-schedule.ward', $nurse->ward_assignment) }}" class="text-blue-600 hover:underline">
                            {{ $nurse->ward_assignment }}
                        </a>
                        @php
                            $ward = \App\Models\Ward::where('name', $nurse->ward_assignment)->first();
                        @endphp
                        <a href="{{ route('bed-management.bed-map', ['ward_id' => $ward ? $ward->id : '']) }}" class="ml-2 text-sm text-indigo-600 hover:underline">
                            (View Bed Map)
                        </a>
                    </span>
                </div>
                <div>
                    <span class="block text-sm font-medium text-gray-700">Primary Shift</span>
                    <span class="block font-semibold">
                        @if($nurse->shift == 'Morning')
                            Morning (7:00 AM - 3:00 PM)
                        @elseif($nurse->shift == 'Evening')
                            Evening (3:00 PM - 11:00 PM)
                        @elseif($nurse->shift == 'Night')
                            Night (11:00 PM - 7:00 AM)
                        @else
                            {{ $nurse->shift }}
                        @endif
                    </span>
                </div>
                <div>
                    <span class="block text-sm font-medium text-gray-700">Status</span>
                    <span class="block font-semibold">{{ $nurse->status }}</span>
                </div>
            </div>
        </div>
        
        <!-- Date Filter -->
        <div class="mb-6 p-4 bg-gray-50 rounded-md">
            <form action="{{ route('shift-schedule.nurse', $nurse->id) }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label for="start_date" class="block text-sm font-medium text-gray-700 mb-1">Start Date</label>
                    <input type="date" name="start_date" id="start_date" value="{{ $startDate->format('Y-m-d') }}" class="w-full rounded-md border-gray-300 shadow-sm">
                </div>
                
                <div>
                    <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">End Date</label>
                    <input type="date" name="end_date" id="end_date" value="{{ $endDate->format('Y-m-d') }}" class="w-full rounded-md border-gray-300 shadow-sm">
                </div>
                
                <div class="flex items-end">
                    <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">
                        Apply Filter
                    </button>
                </div>
            </form>
        </div>
        
        <!-- Weekly Calendar View -->
        <div class="mb-6">
            <h3 class="text-lg font-semibold mb-4">Weekly Schedule</h3>
            
            <div class="overflow-x-auto">
                <table class="min-w-full border border-gray-200 bg-white">
                    <thead>
                        <tr>
                            @foreach($dateRange as $date)
                                <th class="py-2 px-3 bg-gray-50 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-b">
                                    <div>{{ $date->format('D') }}</div>
                                    <div>{{ $date->format('M d') }}</div>
                                </th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            @foreach($dateRange as $date)
                                <td class="py-2 px-3 border align-top">
                                    <div class="min-h-[150px]">
                                        @php
                                            $daySchedules = $schedulesByDate[$date->format('Y-m-d')] ?? collect();
                                        @endphp
                                        
                                        @forelse($daySchedules as $schedule)
                                            <div class="mb-2 p-2 rounded text-xs 
                                                @if($schedule->shift == 'Morning') bg-blue-100 text-blue-800
                                                @elseif($schedule->shift == 'Evening') bg-indigo-100 text-indigo-800
                                                @elseif($schedule->shift == 'Night') bg-purple-100 text-purple-800
                                                @else bg-gray-100 text-gray-800
                                                @endif">
                                                <div class="font-medium">
                                                    @if($schedule->shift == 'Morning')
                                                        Morning Shift (7:00 AM - 3:00 PM)
                                                    @elseif($schedule->shift == 'Evening')
                                                        Evening Shift (3:00 PM - 11:00 PM)
                                                    @elseif($schedule->shift == 'Night')
                                                        Night Shift (11:00 PM - 7:00 AM)
                                                    @else
                                                        {{ $schedule->shift }} Shift
                                                    @endif
                                                </div>
                                                <div>{{ $schedule->start_time->format('H:i') }} - {{ $schedule->end_time->format('H:i') }}</div>
                                                <div class="mt-1">
                                                    <a href="{{ route('shift-schedule.ward', $schedule->ward->id) }}" class="
                                                        @if($schedule->shift == 'Morning') text-blue-600
                                                        @elseif($schedule->shift == 'Evening') text-indigo-600
                                                        @elseif($schedule->shift == 'Night') text-purple-600
                                                        @else text-gray-600
                                                        @endif hover:underline">
                                                        {{ $schedule->ward->name }} Ward
                                                    </a>
                                                    <a href="{{ route('bed-management.bed-map', ['ward_id' => $schedule->ward->id]) }}" class="ml-1 text-xs
                                                        @if($schedule->shift == 'Morning') text-blue-600
                                                        @elseif($schedule->shift == 'Evening') text-indigo-600
                                                        @elseif($schedule->shift == 'Night') text-purple-600
                                                        @else text-gray-600
                                                        @endif hover:underline">
                                                        (Bed Map)
                                                    </a>
                                                </div>
                                                <div class="mt-1">
                                                    <span class="px-1 py-0.5 rounded
                                                        @if($schedule->shift == 'Morning') bg-blue-50
                                                        @elseif($schedule->shift == 'Evening') bg-indigo-50
                                                        @elseif($schedule->shift == 'Night') bg-purple-50
                                                        @else bg-gray-50
                                                        @endif">
                                                        {{ ucfirst($schedule->status) }}
                                                    </span>
                                                </div>
                                                <div class="flex space-x-1 mt-2">
                                                    <a href="{{ route('shift-schedule.edit', $schedule->id) }}" class="
                                                        @if($schedule->shift == 'Morning') text-blue-600
                                                        @elseif($schedule->shift == 'Evening') text-indigo-600
                                                        @elseif($schedule->shift == 'Night') text-purple-600
                                                        @else text-gray-600
                                                        @endif hover:underline">
                                                        Edit
                                                    </a>
                                                </div>
                                            </div>
                                        @empty
                                            <div class="text-center text-gray-400 text-xs pt-4">
                                                No shifts scheduled
                                            </div>
                                        @endforelse
                                    </div>
                                </td>
                            @endforeach
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        
        <!-- List View -->
        <div>
            <h3 class="text-lg font-semibold mb-4">All Scheduled Shifts</h3>
            
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white border border-gray-200">
                    <thead>
                        <tr class="bg-gray-50">
                            <th class="py-2 px-4 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date</th>
                            <th class="py-2 px-4 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Shift</th>
                            <th class="py-2 px-4 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Time</th>
                            <th class="py-2 px-4 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ward</th>
                            <th class="py-2 px-4 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                            <th class="py-2 px-4 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @php
                            $allSchedules = collect();
                            foreach ($schedulesByDate as $daySchedules) {
                                $allSchedules = $allSchedules->merge($daySchedules);
                            }
                            $allSchedules = $allSchedules->sortBy([
                                ['schedule_date', 'asc'],
                                ['start_time', 'asc'],
                            ]);
                        @endphp
                        
                        @forelse($allSchedules as $schedule)
                            <tr class="hover:bg-gray-50">
                                <td class="py-2 px-4 text-sm">{{ $schedule->schedule_date->format('M d, Y') }}</td>
                                <td class="py-2 px-4 text-sm">
                                    <span class="px-2 py-1 text-xs rounded-full 
                                        @if($schedule->shift == 'Morning') bg-blue-100 text-blue-800
                                        @elseif($schedule->shift == 'Evening') bg-indigo-100 text-indigo-800
                                        @elseif($schedule->shift == 'Night') bg-purple-100 text-purple-800
                                        @else bg-gray-100 text-gray-800
                                        @endif">
                                        @if($schedule->shift == 'Morning')
                                            Morning (7:00 AM - 3:00 PM)
                                        @elseif($schedule->shift == 'Evening')
                                            Evening (3:00 PM - 11:00 PM)
                                        @elseif($schedule->shift == 'Night')
                                            Night (11:00 PM - 7:00 AM)
                                        @else
                                            {{ $schedule->shift }}
                                        @endif
                                    </span>
                                </td>
                                <td class="py-2 px-4 text-sm">{{ $schedule->start_time->format('H:i') }} - {{ $schedule->end_time->format('H:i') }}</td>
                                <td class="py-2 px-4 text-sm">
                                    <a href="{{ route('shift-schedule.ward', $schedule->ward->id) }}" class="text-blue-600 hover:underline">
                                        {{ $schedule->ward->name }}
                                    </a>
                                    <a href="{{ route('bed-management.bed-map', ['ward_id' => $schedule->ward->id]) }}" class="ml-1 text-xs text-indigo-600 hover:underline">
                                        (Bed Map)
                                    </a>
                                </td>
                                <td class="py-2 px-4 text-sm">
                                    <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">
                                        Confirmed
                                    </span>
                                </td>
                                <td class="py-2 px-4 text-sm">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('shift-schedule.edit', $schedule->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                        <form action="{{ route('shift-schedule.destroy', $schedule->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this shift?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:text-red-900">Delete</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="py-4 px-4 text-center text-gray-500">
                                    No shift schedules found for this nurse in the selected date range.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script>
    // Check if URL has no date parameters and set defaults
    document.addEventListener('DOMContentLoaded', function() {
        const urlParams = new URLSearchParams(window.location.search);
        // If no date parameters in URL, the controller already set the defaults
        // This helps with visual feedback on the form
    });
</script>
@endpush 